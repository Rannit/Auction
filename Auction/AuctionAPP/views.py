...
def save_auction(request):
    if request.user.is_authenticated():
        ...
        if option == 'Yes':
            ...
            a.save()
            send_mail('New auction created.', "Your new auction has been created successfully.", 'no_repli@auction.com', [request.user.email,], fail_silently=False)
...
def ban_auction(request, id):
    if request.user.is_superuser:
        ...
        if auct:
            ...
            auct.lifecycle = 'B'
            auct.save()
            send_mail('Auction Banned.', "Your auction has been baned.", 'no_repli@auction.com', [auct.seller.email,], fail_silently=False)
            ...
            #print mails
            send_mail('Auction Banned.', "An auction in which you have bid on has been banned.", 'no_repli@auction.com', mails, fail_silently=False)
    ...
    if request.user.is_authenticated():
        if request.method == 'POST':
            ...
            if prev_bid_wining:
                ...
                send_mail('Bid losing.', "Somebody bid in the same auction that you did.", 'no_repli@auction.com', [prev_bid_wining.user.email,], fail_silently=False)
            ...
            send_mail('New bid in your auction.', "Somebody bid in the one of your auctions.", 'no_repli@auction.com', [auct.seller.email,], fail_silently=False)
            send_mail('Bid accepted.', "Your new bed has been acepted.", 'no_repli@auction.com', [request.user.email,], fail_silently=False)
